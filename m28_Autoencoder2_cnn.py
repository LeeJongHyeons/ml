...
decoded = Conv2D(1,(3,3), activation='sigmoid', loss='binary_crossentropy')

encode_model = Dense(encoding_dim, activation='relu')(input_img)

# decode는 입력의 손실있는 재구성 (lossy reconstruction)
decode_model = Dense(784, activation = 'sigmoid')(encode_model)
# decode = Dense(784, activation = 'relu')(encode_model)

# 입력을 입력의 재구성으로 매피할 모델 784 -> 32 -> 784
autoencoder = Model(input_img, decode_model)

# 입력된 인코딩된 입력의 표현으로 매핑 784 -> 32 (Incoder)
...
# 디 코더 모델 생성 
decoder = Model(encode_input, decode_layer(encode_input)) # 32 -> 784 (Decoder)

autoencoder.summary() # 둘이 합침
encoder.summary()
decoder.summary()
...
for i in range(number):
    ...
    # 재지정 데이터
    ax  = plt.subplot(2, number, i + number +1)
